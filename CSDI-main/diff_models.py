...
class diff_CSDI(nn.Module):
    def __init__(self, config, inputdim=2):
        ...
        # inputdim: dimensionality of input sequence

        super().__init__()
        self.channels = config["channels"]

        self.diffusion_embedding = DiffusionEmbedding( # embedding for diffusion process
            num_steps=config["num_steps"],
            embedding_dim=config["diffusion_embedding_dim"],
        )

        self.input_projection = Conv1d_with_init(inputdim, self.channels, 1) # maps inputdim to number of channels

        # these process the outputs
        self.output_projection1 = Conv1d_with_init(self.channels, self.channels, 1) # projects hidden representation
        ...
        self.residual_layers = nn.ModuleList( 
            [
                ResidualBlock( # reisdual blocks that apply various transformations for better learning
                    side_dim=config["side_dim"], # feed it all the inputs
                    channels=self.channels,
                    diffusion_embedding_dim=config["diffusion_embedding_dim"],
                    nheads=config["nheads"],
                    is_linear=config["is_linear"],
                )
                for _ in range(config["layers"]) # number of residual layers requested
            ]
        )

    # foward pss for model learning
